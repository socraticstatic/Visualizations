...
import { simulateRgb } from "@/charts/palette/cvd";

/**
 * Regression guard: the severity-1.0 simulation must match the published
...
describe("simulateRgb matches the published Machado 2009 matrices", () => {
  for (const type of ["protan", "deutan", "tritan"] as const) {
    it(type, () => {
      for (const probe of PROBES) {
        const got = simulateRgb({ r: probe[0], g: probe[1], b: probe[2] }, type, 1);
  ...
  it("deuteranopia collapses red toward olive (red must not stay red)", () => {
    const red = simulateRgb({ r: 1, g: 0, b: 0 }, "deutan", 1);
