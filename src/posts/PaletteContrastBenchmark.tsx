...
/**
 * Every figure measured off a published palette is computed at render time from
 * the same engine the builder uses, against the same BENCHMARKS constants.
 * Nothing is transcribed, so those cannot drift from the engine.
 *
 * The solver's own figures are the exception, and they are read from
 * benchmarkFacts.json instead. Rendering them live meant publishing a different
 * number to every JavaScript engine - see src/posts/benchmarkFacts.ts. They are
 * regenerated by `npm run freeze:benchmark`, which is enforced against
 * PALETTE_VERSION by a test, so they still cannot go stale unnoticed.
 */
...
/** Small counts read as words in this post's prose, not as digits. */
const countWord = (n: number) => COUNT_WORDS[n] ?? String(n);

/** "A", "A and B", "A, B and C" - the list the sentence needs, not an array. */
...
export default function PaletteContrastBenchmark() {
  const facts = useMemo(() => {
    ...
    return {
      ...
      // Frozen at build time, not solved here. See the note at the top.
      solved6Worst: FROZEN.solved6.worstContrast,
      solved6Cvd: FROZEN.solved6.worstCvdDeltaE,
      solved6Pair: FROZEN.solved6.minPairDeltaE,
      // How the solver actually places against the published palettes is
      // counted, not asserted. The prose said "beats four of them" and went on
      // saying it after the figure moved, which is the failure mode a sentence
      // full of hand-typed numbers always has.
      ...(() => {
        const solvedPair = FROZEN.solved6.minPairDeltaE;
        const pairs = BENCHMARKS.map((b) => ({
          name: b.name,
          pair: minPairDeltaE(b.hexes.slice(0, 6).map(fromCss)),
        }));
        const losesTo = pairs.filter((p) => p.pair > solvedPair).sort((a, b) => a.pair - b.pair);
        return {
          solved6Beats: pairs.length - losesTo.length,
          solved6LosesTo: joinNames(losesTo.map((p) => p.name)),
        };
      })(),
    };
  }, []);
    ...
    <article className="pb-4">
      ...
      <Prose>
        ...
        <P>
          ...
          {facts.solved6Pair.toFixed(1)} beats {countWord(facts.solved6Beats)} of them and loses to{" "}
          {facts.solved6LosesTo}, which spread their hues further than my solver does.
        </P>
        <P>
